...
// Porte fiel da Edge Function `gerar-conclusao-drps-ia` (Supabase/Deno → Next.js).
// Gera a Conclusão técnica do DRPS via Groq. Mesma lógica de negócio.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!body?.setor?.nome || !Array.isArray(body?.topicos)) {
      return NextResponse.json(
        { error: "Payload inválido: 'setor.nome' e 'topicos[]' são obrigatórios" },
        { status: 400 }
      );
    }

    // Conclusão por setor → texto corrido em parágrafos (SYSTEM_PROMPT_SETOR).
    // Conclusão geral (consolidada) → texto corrido (SYSTEM_PROMPT).
    ...
    return NextResponse.json({ data: { conclusao } }, { status: 200 });
  } catch (e) {
    return NextResponse.json(
