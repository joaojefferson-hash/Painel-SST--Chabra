...
// Porte fiel da Supabase Edge Function `gerar-parecer-aep-ia`.
// Gera Parecer Técnico Preliminar ou Recomendações para um setor da AEP,
  ...
  try {
    ...
    if (!body?.setor_nome || !body?.campo) {
      return NextResponse.json(
        { error: "setor_nome e campo são obrigatórios" },
        { status: 400 }
      );
    }
