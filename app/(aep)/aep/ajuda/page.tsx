...
const ORGANIZACIONAL_ITENS: ItemData[] = [
  ...
  {
    label: "Falta de autonomia nas decisões do trabalho",
    conceito: `A autonomia — o grau em que o trabalhador controla como, quando e com que métodos executa seu trabalho — é um dos recursos de trabalho mais poderosos identificados pela pesquisa em saúde ocupacional. No modelo de Karasek (Demanda-Controle), a combinação de alta demanda com baixa autonomia é chamada de "trabalho de alto risco" e está associada ao dobro do risco cardiovascular em relação a trabalhadores com controle equivalente à sua demanda. A autonomia não é apenas conforto — é um mecanismo fisiológico de regulação do estresse: quando o trabalhador percebe que tem controle sobre sua situação, a resposta de ameaça do sistema nervoso autônomo é atenuada. A ausência completa de controle sobre o próprio trabalho é, em si, um estressor independente do volume de tarefas.`,
    como: [
      "Mapeie as decisões que o trabalhador pode tomar autonomamente: pode ajustar a sequência de suas tarefas? Pode adaptar o método de execução? Pode decidir quando fazer pausas? Pode resolver problemas rotineiros sem aprovação do gestor? Quanto mais 'nãos', menor a autonomia.",
      "Aplique a escala de Karasek simplificada (2 perguntas): 'Você tem liberdade para decidir como fazer seu trabalho?' e 'Você pode influenciar a quantidade de trabalho que precisa fazer?' Ambas avaliadas de 1 (nunca) a 4 (frequentemente). Score total ≤ 4 indica baixo controle.",
      "Observe o grau de prescrição do trabalho: há procedimentos detalhados para cada etapa, sem margem de adaptação? Sistemas com monitoramento em tempo real de cada ação (call center com scripts obrigatórios, linhas com takt time rigidamente cronometrado) tendem a zero autonomia.",
      "Investigue se há penalização por adaptações: 'Se você encontrar um jeito melhor de fazer algo, pode mudar como você trabalha?' Organizações que punem iniciativa — mesmo bem-intencionada — destroem a autonomia mesmo quando o procedimento formal prevê abertura.",
      "Contextualize com a função: autonomia zero é esperada em atividades de alto risco regulamentado (piloto de avião, operador nuclear). Em trabalhos de serviço, operacional e administrativo, algum grau de controle sobre o próprio trabalho é ergonomicamente desejável e organizacionalmente viável.",
    ],
    atencao: "Erro comum: confundir padronização necessária com ausência de autonomia. Procedimentos claros são um recurso de trabalho positivo (reduzem incerteza); o que é deletério é a padronização que não deixa margem alguma para o trabalhador adaptar o método às variações reais da tarefa.",
    marque_sim: "Se o trabalhador não pode ajustar a sequência ou o método de trabalho, não pode resolver problemas rotineiros sem autorização, o trabalho é inteiramente prescrito por sistema ou máquina, ou há relatos de punição por iniciativas adaptativas independente do resultado.",
  },
  {
    label: "Falta de reconhecimento / valorização",
    conceito: `O modelo do Desequilíbrio Esforço-Recompensa (ERI) de Siegrist postula que o estresse no trabalho surge quando o esforço investido pelo trabalhador é sistematicamente desproporcional à recompensa recebida — e a recompensa não é apenas financeira. A recompensa psicossocial inclui reconhecimento (feedback positivo, elogio, respeito), oportunidade de desenvolvimento (promoção, aprendizado, status) e segurança. Estudos longitudinais mostram que trabalhadores em alto desequilíbrio esforço-recompensa têm 2–3 vezes mais risco de depressão e doenças cardiovasculares em relação aos em equilíbrio. A privação de reconhecimento ativa circuitos neurais de rejeição social que têm efeito motivacionalmente devastador a longo prazo — o trabalhador para de se engajar, porque o engajamento não é recompensado.`,
    como: [
      "Pergunte diretamente em entrevista individual: 'Quando você faz um bom trabalho, você recebe algum feedback? De quem e com que frequência?' e 'Você sente que seu esforço é reconhecido aqui?' Respostas como 'só ouço quando erro' são indicadores fortes.",
      "Explore o reconhecimento não financeiro: há reuniões de feedback estruturadas? Há algum mecanismo de destacar bom desempenho? O gestor agradece ou parabeniza publicamente ou em privado? A ausência total de reconhecimento não financeiro é um indicador mesmo quando o salário é adequado.",
      "Avalie o equilíbrio esforço-recompensa: compare o nível de exigência da função (responsabilidade, complexidade, riscos assumidos) com a posição salarial no mercado, benefícios, e perspectiva de desenvolvimento. Função de alta responsabilidade com salário e benefícios abaixo do mercado e sem perspectiva de progressão é alto desequilíbrio.",
      "Identifique assimetria de feedback: em muitos ambientes, erros são documentados e discutidos formalmente, enquanto acertos são ignorados. Um sistema de gestão de desempenho que só registra falhas cria percepção de reconhecimento exclusivamente negativo.",
      "Verifique o histórico de promoções: quantos trabalhadores foram promovidos nos últimos 2 anos neste setor? A progressão existe ou os trabalhadores percebem um teto? Stagnação percebida em trabalhadores de bom desempenho é um indicador de risco ERI.",
    ],
    atencao: "Erro comum: assumir que bom salário substitui reconhecimento. Pesquisas em motivação (Herzberg) distinguem fatores higiênicos (salário, benefícios — cuja ausência gera insatisfação, mas cuja presença não gera motivação) dos fatores motivadores (reconhecimento, realização, responsabilidade). Os dois são necessários.",
    marque_sim: "Se ≥ 2 trabalhadores relatam ausência de feedback positivo por bom desempenho, há desequilíbrio evidente entre exigência da função e recompensa (salarial + desenvolvimento + status), ou o sistema de gestão de desempenho registra apenas falhas sem documentar conquistas.",
  },
  {
    label: "Ambiguidade ou conflito de funções",
    conceito: `A ambiguidade de papel existe quando o trabalhador não tem clareza suficiente sobre o que é esperado dele — quais são suas responsabilidades, com que critério será avaliado, qual é seu nível de autoridade, a quem deve reportar em cada situação. O conflito de papel existe quando o trabalhador recebe demandas contraditórias de fontes diferentes — dois gestores com instruções opostas, demanda da chefia versus exigência do cliente, política formal versus prática informal. Ambas as condições são estressores ocupacionais comprovados que aumentam a ansiedade, reduzem o comprometimento e pioram o desempenho. A ambiguidade é especialmente comum em organizações que cresceram rápido sem formalizar processos, em cargos recém-criados e em transições organizacionais (fusões, reestruturações).`,
    como: [
      ...
      "Verifique se existe descrição de cargo escrita e atualizada: solicite ao RH. Ausência de job description é um indicador estrutural de ambiguidade de papel. Se existe, compare com o que o trabalhador relata que faz — as divergências são informativas.",
      "Investigue situações de conflito de demandas: 'Já aconteceu de você receber instruções diferentes de pessoas diferentes sobre a mesma tarefa? O que você faz nesses casos?' Incapacidade de responder, ou relato de paralisação frequente nesses momentos, indica conflito de papel habitual.",
      "Mapeie a cadeia de reporte: o trabalhador reporta para um gestor direto ou para múltiplos? Em estruturas matriciais (comum em projetos e consultorias), o conflito de papel é endêmico — verifique se há protocolo claro de priorização entre demandas concorrentes.",
      "Avalie novos trabalhadores: ambiguidade de papel é mais intensa no primeiro ano. Pergunte ao trabalhador há quanto tempo está na função — se relata ainda ter dificuldade em entender o que é esperado dele após > 6 meses, há problema estrutural de onboarding e definição de papéis.",
    ],
    atencao: "Erro comum: interpretar trabalhadores proativos que 'resolvem tudo' como indicadores de organização saudável. Trabalhadores que se tornaram o 'ponto de solução' informal de uma área frequentemente estão sofrendo acúmulo de papel não formalizado — fazem o trabalho de vários sem reconhecimento ou compensação.",
    marque_sim: "Se o trabalhador não consegue descrever claramente suas responsabilidades e critérios de avaliação, há relatos de instruções contraditórias de fontes diferentes, ausência de descrição de cargo formalizada, ou acúmulo de funções não previsto na contratação.",
  },
  {
    label: "Comunicação organizacional deficiente",
    conceito: `A comunicação organizacional deficiente opera em dois sentidos com consequências distintas. No sentido descendente (gestão → trabalhador), a falta de informação sobre metas, mudanças, expectativas e contexto da empresa cria incerteza crônica — e incerteza é um estressor potente, pois o cérebro humano responde à ambiguidade com ativação do sistema de ameaça mesmo na ausência de perigo concreto. No sentido ascendente (trabalhador → gestão), a ausência de canais efetivos de comunicação significa que problemas reais de segurança, qualidade e bem-estar não chegam a quem pode resolvê-los. O resultado prático é que a gestão toma decisões com informação incompleta enquanto os trabalhadores sentem que suas preocupações não importam — uma combinação que deteriora rapidamente o clima organizacional e a segurança.`,
    como: [
      "Avalie a comunicação descendente: com que frequência e por que canais os trabalhadores recebem informações sobre as metas da empresa, mudanças em processos, resultados e contexto do negócio? Pergunte: 'Como você fica sabendo das mudanças que acontecem na empresa?' Respostas como 'pelo corredor' ou 'quando já foi implementado' indicam falha.",
      "Avalie a comunicação ascendente: há mecanismo formal para trabalhadores reportarem problemas de segurança, sugerir melhorias ou comunicar preocupações sem medo de represália? Canal de sugestões, DDS (Diálogo Diário de Segurança), SIPAT com espaço real para escuta — verificar se existem e se são utilizados.",
      "Observe a qualidade das instruções operacionais: procedimentos escritos existem? São compreensíveis para o nível educacional dos trabalhadores? Estão atualizados com a prática real? Instruções que ninguém lê porque 'não correspondem ao que fazemos de verdade' indicam ruptura entre comunicação formal e informal.",
      "Identifique silos de informação: departamentos que não se comunicam sobre processos interdependentes criam pontos de falha. Pergunte: 'Quando há um problema que envolve outro departamento, como vocês se comunicam? Funciona?' Respostas que descrevem conflitos crônicos de passagem de serviço indicam silo.",
      "Avalie o feedback sobre desempenho: o trabalhador sabe periodicamente como está seu desempenho? O feedback é específico e acionável, ou genérico e raro? Ausência de feedback estruturado é um indicador de comunicação descendente deficiente mesmo quando não há conflito explícito.",
    ],
    atencao: "Erro comum: confundir quantidade de comunicação com qualidade. Organizações com reuniões excessivas, e-mails constantes e grupos de WhatsApp com dezenas de mensagens por dia podem ter comunicação deficiente — ruído não é informação. O problema frequente é excesso de comunicação operacional e ausência de comunicação estratégica e de saúde/segurança.",
    marque_sim: "Se trabalhadores descobrem mudanças importantes por canais informais, não há canal efetivo para comunicação ascendente de problemas, há relatos de desinformação crônica sobre expectativas e metas, ou procedimentos formais não correspondem à prática real por falta de atualização.",
  },
  {
    label: "Violência / agressão de clientes ou terceiros",
    conceito: `A violência no trabalho originada por clientes, usuários ou terceiros — chamada de violência de Tipo II na classificação internacional — é a modalidade mais comum de violência ocupacional e a mais subestimada. Abrange desde agressão verbal (xingamentos, humilhações, ameaças) até agressão física. Trabalhadores de saúde, segurança pública, transporte público, serviços de atendimento ao público, educação e varejo são os grupos mais expostos. A violência verbal é frequentemente tratada como "parte do trabalho" e não é registrada — o que é um erro técnico e um problema de saúde: mesmo a violência verbal crônica está associada a burnout, TEPT (Transtorno de Estresse Pós-Traumático) e afastamentos por transtornos mentais. O silêncio institucional sobre violência de clientes é, em si, um fator agravante do dano.`,
    como: [
      "Pergunte diretamente e sem eufemismos: 'Você já sofreu ou presenciou situação de agressão verbal — xingamento, humilhação, ameaça — de cliente, usuário ou terceiro durante o trabalho?' e 'Com que frequência isso acontece?' A normalização é grande — muitos trabalhadores só relatam se a pergunta for direta.",
      "Verifique os registros da empresa: há livro de ocorrências, SIPAT com registros de violência, ou BO registrado em algum momento? A ausência total de registros em setores de atendimento ao público não significa ausência de episódios — significa ausência de cultura de registro.",
      "Avalie as medidas de proteção existentes: há protocolo de como agir em caso de agressão? Há treinamento para situações de conflito com clientes? Há suporte psicológico para trabalhadores após incidentes? Ausência de todas essas medidas em setor de atendimento ao público é uma lacuna técnica relevante.",
      "Identifique fatores que ampliam o risco: trabalho com público em situação de vulnerabilidade (saúde, assistência social), atendimento a pessoas sob efeito de álcool ou drogas, setor com histórico de longas filas e demora (agências bancárias, postos de saúde), cobrança ou execução de regras impopulares.",
      "Avalie o impacto nos trabalhadores expostos: pergunte sobre sintomas de ansiedade antecipatória ('você se sente ansioso antes de começar o turno por medo de situações difíceis com clientes?'), alterações de sono, ou pensamentos intrusivos sobre incidentes passados — sinais de resposta traumática.",
    ],
    atencao: "Erro comum: limitar a avaliação a agressões físicas formalmente registradas. Em termos de saúde mental ocupacional, a agressão verbal crônica e o assédio de clientes têm impacto comparável ou superior ao de um incidente físico único — porque é cumulativo, constante e raramente reconhecido como um risco real.",
    marque_sim: "Se há relatos de episódios de agressão verbal ou física por clientes/terceiros, setor tem características de alto risco (atendimento ao público vulnerável, cobranças, restrições impopulares), ausência de protocolo de proteção e suporte pós-incidente, ou trabalhadores relatam ansiedade antecipatória relacionada ao contato com o público.",
  },
  {
    label: "Excesso de controle e monitoramento",
    conceito: `O monitoramento eletrônico e comportamental do trabalhador se expandiu drasticamente com a digitalização do trabalho: sistemas que registram cada tecla pressionada, rastreamento por GPS em tempo real, monitoramento de tela por câmeras remotas, métricas de produtividade por minuto em call centers, câmeras de vigilância com reconhecimento facial. O problema não é o monitoramento em si — é o monitoramento que priva o trabalhador de qualquer zona de autonomia percebida e que comunica, implicitamente, que o trabalhador não é confiável. A percepção crônica de vigilância permanente é um estressor por si só, independente do comportamento do trabalhador: ativa a resposta de vigilância do sistema nervoso autônomo de forma contínua, com os mesmos efeitos fisiológicos de ameaça percebida. O excesso de controle também destrói a criatividade e a iniciativa — o trabalhador aprende a fazer exatamente o prescrito e nada mais.`,
    como: [
      "Identifique os mecanismos de monitoramento em uso: quais são os sistemas que registram produtividade, localização, comunicação ou comportamento do trabalhador? Com que granularidade (por turno? por hora? por minuto?). Quem tem acesso a esses dados e com que finalidade?",
      "Avalie a transparência: os trabalhadores sabem que estão sendo monitorados, por quais sistemas, que dados são coletados e quem os acessa? O monitoramento transparente com finalidade clara (ex.: segurança em área de risco) tem impacto diferente do monitoramento opaco.",
      "Investigue o uso dos dados: os resultados do monitoramento são utilizados apenas para punição e controle, ou também para identificar gargalos de processo, fornecer feedback de desenvolvimento e melhorar condições? O uso exclusivamente punitivo é o padrão mais danoso.",
      "Pergunte sobre a percepção: 'Você sente que é confiado para fazer seu trabalho ou que precisa provar constantemente que está trabalhando?' A percepção de vigilância permanente é o indicador mais relevante — mais do que o sistema técnico em si.",
      "Avalie o microgerenciamento comportamental: gestores que verificam cada tarefa antes de prosseguir, aprovam cada e-mail, estão copiados em toda comunicação, ou perguntam constantemente 'onde você está nessa tarefa?' — isso é microgerenciamento que produz o mesmo efeito de excesso de controle mesmo sem sistema eletrônico.",
    ],
    atencao: "Erro comum: interpretar sistemas de monitoramento extensivos como prova de gestão profissional. Em muitos casos, o excesso de monitoramento compensa a ausência de sistemas de gestão por resultados — é mais fácil medir se o trabalhador está na cadeira do que se está entregando valor. O resultado é trabalhador controlado, não trabalhador produtivo.",
    marque_sim: "Se há monitoramento contínuo de atividade com granularidade de minutos ou segundos, trabalhadores relatam percepção de vigilância permanente sem margem de privacidade, o monitoramento é opaco (trabalhadores não sabem o que é coletado), ou os dados são usados exclusivamente para controle e punição.",
  },
  {
    label: "Insegurança quanto à continuidade do emprego",
    conceito: `A insegurança de emprego — a percepção subjetiva de ameaça ao emprego atual — é um dos estressores ocupacionais mais bem documentados na literatura epidemiológica. O que é psicologicamente relevante não é necessariamente a ameaça objetiva (demissão iminente) mas a percepção subjetiva de instabilidade, que pode existir mesmo em ambientes com baixa rotatividade real, quando a comunicação da gestão é ambígua ou quando há histórico de demissões em massa anteriores. A insegurança crônica ativa o eixo do estresse de forma contínua — cortisol, hiperatividade do sistema nervoso simpático, distúrbios do sono — com os mesmos efeitos da ameaça de qualquer natureza. Estudos mostram que a insegurança de emprego tem impacto comparável ao desemprego real sobre a saúde mental, com pior prognóstico em alguns indicadores porque combina o estresse da ameaça com a obrigação de continuar trabalhando normalmente.`,
    como: [
      "Pergunte diretamente: 'Você se preocupa com a possibilidade de perder seu emprego?' e 'Você se sente estável nesta empresa?' Avalie a resposta em contexto — uma empresa sólida com comunicação clara pode ter trabalhadores com alta insegurança por razões históricas ou de cultura.",
      "Verifique o histórico recente: houve demissões em massa, reestruturações, fusões ou aquisições nos últimos 2 anos? Mesmo trabalhadores que sobreviveram a esses eventos costumam desenvolver insegurança crônica — o que a literatura chama de 'sobrevivente da demissão' (survivor syndrome).",
      "Avalie a precariedade contratual: qual a proporção de trabalhadores com contrato temporário, terceirizados ou PJ em relação ao quadro permanente? Contratos precários com renovação periódica são, por definição, fontes estruturais de insegurança.",
      "Investigue a comunicação da gestão sobre o futuro: a empresa comunica seus resultados, perspectivas e estratégias para os trabalhadores? Gestão opaca que não informa sobre a situação da empresa deixa espaço para rumores e ansiedade — especialmente em setores em transformação tecnológica ou retração de mercado.",
      "Avalie o impacto nos comportamentos: trabalhadores com alta insegurança frequentemente 'se escondem' — evitam expressar opiniões, nunca questionam decisões, fazem horas extras não solicitadas para demonstrar valor. Esse comportamento de hiperconformidade tem custo de saúde mental e mascara problemas para a gestão.",
    ],
    atencao: "Erro comum: avaliar insegurança de emprego apenas em empresas com dificuldades financeiras visíveis. Grandes empresas em crescimento também têm trabalhadores com alta insegurança — especialmente quando há histórico de reestruturações, quando a gestão não comunica perspectivas, ou quando há digitalização/automação em curso sem clareza sobre o futuro dos postos.",
    marque_sim: "Se há histórico recente de demissões em massa ou reestruturações, proporção relevante de contratos precários ou temporários, comunicação opaca sobre a situação e perspectivas da empresa, ou trabalhadores relatam preocupação frequente com a continuidade do emprego.",
  },
  {
    label: "Mudanças organizacionais sem participação dos trabalhadores",
    conceito: `Mudanças organizacionais — reestruturações, implantação de novos sistemas, alteração de processos, mudança de gestão, fusões — são eventos de alta demanda psicossocial porque combinam incerteza (o futuro é desconhecido), perda de controle (o trabalhador não decide sobre algo que afeta profundamente sua vida) e, frequentemente, acréscimo de carga durante a transição. A participação dos trabalhadores no processo de mudança não é apenas eticamente desejável — é tecnicamente mais eficaz: trabalhadores que conhecem o processo real identificam problemas de implementação que gestores e consultores externos não percebem, e trabalhadores que participaram do planejamento têm significativamente menos resistência à mudança. A NR-01 revisada (GRO/PGR) reconhece explicitamente a gestão de mudanças como elemento do gerenciamento de riscos ocupacionais.`,
    como: [
      "Identifique mudanças relevantes ocorridas nos últimos 12 meses: novos sistemas ERP/CRM, mudanças de processo, alteração de metas, mudança de gestão, reestruturação de equipes, digitalização de tarefas manuais. Qualquer dessas categorias é elegível para avaliação.",
      "Avalie como a mudança foi comunicada: os trabalhadores foram consultados antes da decisão? Foram informados antes da implementação com antecedência suficiente? Ou descobriram a mudança no dia em que entrou em vigor? O momento da comunicação é um forte indicador da cultura de participação.",
      "Investigue o impacto percebido da mudança mais recente: 'Como foi a adaptação ao novo sistema / nova organização? Houve dificuldades? As preocupações dos trabalhadores foram ouvidas?' Mudanças que geraram dificuldades conhecidas pelos trabalhadores mas não resolvidas pela gestão indicam desconexão.",
      "Verifique se há estrutura de participação: há comitê de segurança/CIPA ativo? Há DDS regular? Há pesquisa de clima? Há mecanismo para trabalhadores proporem melhorias de processo? A ausência dessas estruturas torna a participação em mudanças estruturalmente impossível — não é má vontade pontual.",
      "Avalie o impacto na carga de trabalho durante transições: mudanças de sistema costumam criar sobrecarga temporária enquanto o trabalhador aprende a nova ferramenta sem redução das demandas habituais. Verificar se houve ajuste de carga ou suporte adicional durante períodos de transição.",
    ],
    atencao: "Erro comum: tratar comunicação unilateral como participação. 'Avisamos com 2 semanas de antecedência' não é participação — é notificação. Participação real envolve consulta antes da decisão, oportunidade de influenciar o processo, e canal para reportar dificuldades durante a implementação.",
    marque_sim: "Se mudanças relevantes foram implementadas sem consulta prévia aos trabalhadores, trabalhadores relatam dificuldades não resolvidas de mudanças recentes, não há estrutura formal de participação (CIPA inativa, ausência de canais de sugestão), ou houve sobrecarga durante transição sem ajuste de carga ou suporte adicional.",
  },
];
